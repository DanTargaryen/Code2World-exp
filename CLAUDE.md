...
- ✅ **config-driven 全链路打通**(见 §4.1/§4.2):config.yaml = 完整游戏说明书 + 内嵌机制值,
  既当 code condition(Qwen 编码整份 spec,~1650 token)又驱动采集(game_config 按路径提取
  → procgen options,不重编译)。build_dataset/collect_one 已接 config,弃用旧 variants 重编译。
- ✅ **base 单场景训练完成(2026-07-04)**:一条龙 `run_pipeline_base.sh` 在 `hzh-easygo-1n-2`
  跑通 —— 采集 20000 ep → Qwen 编码 spec → 8 卡分片预编码 latents(418942)→ 8 卡 DDP flow
  训练 10000 步。**结果:train fm 1.69→0.025 / eval 0.19→0.023(overfit 成立),
  rollout GT vs 生成高度一致、画面锐利**。ckpt `checkpoints/code2world_base_spec_ddp/`。
- ✅ **评估产物脚本**(见 `docs/eval_artifacts.md`):`plot_loss.py`(loss 曲线)+
  `rollout_sample.py`(ckpt → GT/gen 网格,复用训练验证过的 block_ar_generate + compact 路径)。

### ⏭️ 下一步

1. **code sensitivity(核心命题)**:采多变体(base/fast/lowgrav/no_hazards/invincible,
   configs/ 已就绪)→ 训练 → 验证「改 config 里的规则,生成画面是否跟着变」。
2. **action following** 定量评估:held-out 动作流(eval split),测动作是否被生成帧反映。
3. 多场景(非单 level)提升泛化;或 ablation `--action_mode bias` 对比 crossattn 价值。

> 复跑单变体一条龙:`bash model/run_pipeline_base.sh`(改脚本里 `--variants` 可换变体)。
> 速度提示:1n pod util 常被占满,10000 步约 8h(非代码问题,显存充足无 OOM)。
