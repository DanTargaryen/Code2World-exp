...
each frame). reward summed / done OR-ed over each 4-frame latent window; a window
straddling a reset is dropped so the 4K+1 invariant holds.
"""
...
def collect_unpaired(env_name, n_episodes, max_actions, num_envs, action_repeat):
    ...
    `action_repeat` frames and `action_repeat` per-frame actions; reward summed and
    done OR-ed over the window. ep_a therefore ends with 4K per-frame actions.
    Random levels (num_levels=0). For single fixed scene use collect_single_scene."""
    ...
    # ep_r/ep_d hold K per-latent entries (reward sum / done)
    ...
    def reset_buf(i):
        ep_f[i] = [obs[i].copy()]; ep_a[i] = []; ep_r[i] = []; ep_d[i] = []; ep_k[i] = 0
...
def collect_single_scene(env_name, n_episodes, max_actions, num_envs, action_repeat,
    ...
    advanced across batches so trajectories stay diverse."""
    from procgen import ProcgenEnv
    out_f, out_a, out_r, out_d = [], [], [], []
        ...
        for t in range(max_actions):
            ...
            for _j in range(action_repeat):
                ...
                for i in range(num_envs):
                    ...
                    chunk_r[i] += rew[i]; chunk_done[i] |= bool(done[i])
            for i in range(num_envs):
                if finalized[i]:
                    continue
                all_done = False
                if chunk_done[i]:
                    ...
                    ep_f[i].extend(chunk_f[i]); ep_a[i].extend(chunk_a[i])
                    ep_r[i].append(chunk_r[i]); ep_d[i].append(False); ep_k[i] += 1
            if all_done:
                break
        for i in range(num_envs):
    ...
    return out_f[:n_episodes], out_a[:n_episodes], out_r[:n_episodes], out_d[:n_episodes]
...
def collect_paired(env_name, seeds, max_actions, action_repeat):
    ...
    Each latent window = action_repeat frames (each its own action); reward summed,
    done OR-ed over the window. A window containing a reset is dropped (episode ends
    at last complete latent). ep_a stores per-frame actions (4K for K latents)."""
    from procgen import ProcgenEnv
    ...
    for s in seeds:
        ...
        f = [obs[0].copy()]; a = []; r = []; d = []; k = 0
        for t in range(max_actions):
            chunk_f = []; chunk_a = []; chunk_r = 0.0; chunk_done = False
            for j in range(action_repeat):
                ...
                chunk_r += float(rew[0]); chunk_done |= bool(done[0])
            if chunk_done:
                break                              # drop straddling window; episode ends here
            f.extend(chunk_f); a.extend(chunk_a); r.append(chunk_r); d.append(False); k += 1
        if k >= MIN_ACTIONS:
...
def save_npz(path, fs, as_, rs, ds, seeds=None, action_repeat=1):
    # episode_lengths = K latent steps per episode (= #rewards). actions are PER-FRAME
    # (4K per episode, aligned to frames[1:]); rewards/dones stay per-latent (K).
    lengths = np.array([len(r) for r in rs], np.int32)    # K (latent steps) per episode
    flat_f = np.concatenate(fs, 0)
    flat_a = np.concatenate(as_, 0)
    flat_r = np.concatenate(rs, 0)
    flat_d = np.concatenate(ds, 0)
    assert len(flat_a) == action_repeat * len(flat_r), \
        f"per-frame actions {len(flat_a)} != {action_repeat}*rewards {len(flat_r)}"
...
def main():
    ...
    if args.single_scene:
        ...
        fs, as_, rs, ds = collect_single_scene(args.env, args.unpaired, args.max_steps,
        ...
        fs, as_, rs, ds = collect_single_scene(args.env, args.eval_unpaired, args.max_steps,
                                               args.num_envs, ar, args.level, stream_seed0=900000)
        save_npz(os.path.join(vdir, "episodes_eval.npz"), fs, as_, rs, ds, action_repeat=ar)
