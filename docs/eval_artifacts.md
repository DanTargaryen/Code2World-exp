...
## 2. rollout sample（video）— `model/rollout_sample.py`

加载 ckpt + 一段 eval clip，从 init latent + GT 逐帧动作做 block-AR 生成，GT 与
生成各自 VAE decode 回 RGB，输出**逐帧并排 [GT | gen] 的 mp4 视频**（外加一张
每-latent 网格图便于静态对比）。复用训练里验证过的 `block_ar_generate` + compact 动作
路径，与训练时的 `sample_*.png` 同源。ckpt 超参从 `ckpt["args"]` 自动读取。
...
CUDA_VISIBLE_DEVICES=0 python rollout_sample.py \
  ...
  --variant base --window 20 --n_latents 21 \
  --out ../examples/rollout_final --fps 8 --scale 6
```

产物（`--out` 是目录）：
- `examples/rollout_final/video.mp4` — 逐帧 [左 GT | 右 gen]，81 帧 10.1s（主产物）
- `examples/rollout_final/grid.png` — 21 latent 网格，每格上 GT / 下 gen + 动作标注
